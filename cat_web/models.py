...
#obtner los datos de los diferentes base de datos
def bd_adventurew():
    try:
        qp = text("with t1 as (Select *, p.FirstName+' '+p.LastName AS full_name from [AdventureWorks2017].Person.Person p)   SELECT  full_name as name, em.EmailAddress as email, addr.AddressLine1 as address, addr.PostalCode as zip, pho.PhoneNumber as phone, addr.City as ciudad, cr.Name as pais	   FROM t1 INNER JOIN [AdventureWorks2017].Sales.Customer cu ON cu.PersonID = t1.BusinessEntityID INNER JOIN Person.EmailAddress em ON em.BusinessEntityID = t1.BusinessEntityID  inner join Person.BusinessEntity bus on bus.BusinessEntityID = t1.BusinessEntityID inner join Person.PersonPhone pho on pho.BusinessEntityID = t1.BusinessEntityID INNER join Person.BusinessEntityAddress bea on bea.BusinessEntityID = t1.BusinessEntityID INNER join Person.Address addr on bea.AddressID = addr.AddressID  INNER join sales.SalesTerritory st  on st.TerritoryID = cu.TerritoryID  INNER join Person.CountryRegion cr on cr.CountryRegionCode	= st.CountryRegionCode")
        data_all = db.get_engine(bind='mssql').execute(qp)
        return data_all
...
def bd_dvrental():
    try:
        ...
        db.session.rollback()
        return None
 #metodo para obtner los datos en la rutas       
def datos_por_separados(alz):
    switch = {
        1: bd_adventurew,
        2: bd_dvrental,
    }
    func = switch.get(alz,"Nelseon")
    return func() 
